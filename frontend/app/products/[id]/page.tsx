...
export default function ProductDetailPage() {
  ...
  if (!product) return (
    <div className="text-center py-20">
      <p className="text-2xl font-display text-gray-900 dark:text-white mb-4">Product not found</p>
    ...
    <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid lg:grid-cols-2 gap-10 mb-16">
        ...
        <div className="space-y-4">
          {/* Main image */}
          <div 
            className="aspect-square bg-white dark:bg-base-100 rounded-3xl overflow-hidden glass-card cursor-crosshair relative group"
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            {discount > 0 && <span className="badge bg-danger text-gray-900 dark:text-white">-{discount}% OFF</span>}
            {product.isNewArrival  && <span className="badge bg-success text-gray-900 dark:text-white">New Arrival</span>}
          ...
          <div className="flex items-baseline gap-3">
            <span className="font-display font-black text-4xl text-gray-900 dark:text-white">{formatPrice(product.salePrice)}</span>
            {discount > 0 && (
          ...
          {product.variants.filter(v => v.name && v.name !== 'Default').length > 0 && (
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2 font-medium">Select Variant:</p>
              <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-sm border transition-all ${
                      selectedVar === v.variantId
                        ? 'border-primary-400 bg-primary-400/20 text-gray-900 dark:text-white'
          ...
          <div className="flex items-center gap-4">
            <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">Qty:</p>
            <div className="flex items-center gap-2 glass-card px-1 py-1 rounded-xl">
              <button onClick={() => setQty((q) => Math.max(1, q - 1))}
                className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white hover:bg-white/10 transition-all">
                −
              </button>
              ...
              <tbody>
                {product.specifications.map((spec, idx) => (
                  <tr key={idx} className={idx % 2 === 0 ? 'bg-white/2' : ''}>
                    <td className="px-5 py-3 text-sm text-gray-600 dark:text-gray-400 font-medium w-1/3">{spec.key}</td>
          ...
          <div className="space-y-4">
            {reviews.map((r) => (
              <div key={r._id} className="glass-card p-5">
                ...
                {r.title && <p className="font-semibold text-gray-900 dark:text-white text-sm mb-1">{r.title}</p>}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 sm:p-8 backdrop-blur-sm">
          <div className="bg-white dark:bg-base-100 w-full max-w-6xl h-[85vh] rounded-2xl overflow-hidden flex flex-col lg:flex-row shadow-2xl relative border border-white/10">
            <button 
              onClick={() => setIsModalOpen(false)}
              className="absolute top-4 right-4 z-10 w-10 h-10 flex items-center justify-center rounded-full bg-black/50 hover:bg-black/80 text-gray-900 dark:text-white transition-colors"
            >
              ✕
            </button>
            ...
            {/* Sidebar Thumbnails */}
            <div className="w-full lg:w-[400px] bg-white dark:bg-base-100/95 p-6 flex flex-col border-l border-white/5 overflow-y-auto no-scrollbar">
              <h3 className="text-lg font-display text-gray-900 dark:text-white mb-2 pr-8 leading-tight">{product.title}</h3>
