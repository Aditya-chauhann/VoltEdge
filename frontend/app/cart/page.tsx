...
export default function CartPage() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-24 text-center">
        <div className="glass-card p-12">
          ...
          <h2 className="font-display font-bold text-2xl text-white mb-3">Sign in to view your cart</h2>
          <p className="text-gray-400 mb-6">Your cart items are saved to your account</p>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="font-display font-bold text-3xl text-white mb-8 flex items-center gap-3">
      ...
      {items.length === 0 ? (
        <div className="glass-card p-16 text-center">
          ...
          <p className="font-display font-bold text-white text-2xl mb-3">Your cart is empty</p>
          <p className="text-gray-400 mb-8">Discover amazing electronics and add them to your cart</p>
              ...
              return (
                ...
                >
                  ...
                  <Link href={`/products/${prodId}`} className="flex-shrink-0">
                    <div className="w-24 h-24 bg-base-100 rounded-xl overflow-hidden">
                      {image
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/products/${prodId}`}>
                      <p className="font-medium text-white hover:text-primary-400 transition-colors line-clamp-2 text-sm">{title}</p>
                    </Link>
                    {item.variantId && <p className="text-xs text-gray-400 mt-0.5">{item.variantId}</p>}

                    <div className="flex items-center justify-between mt-3">
                      {/* Qty */}
                      <div className="flex items-center gap-2 glass-card px-1 py-1 rounded-lg">
                        <button
                          disabled={item.qty <= 1 || isLoading}
                          onClick={() => updateItem(item._id, item.qty - 1)}
                          className="w-7 h-7 rounded-md flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all disabled:opacity-40"
                        >
                          <Minus size={12} />
                        </button>
                        ...
                        <button
                          disabled={isLoading}
                          onClick={() => updateItem(item._id, item.qty + 1)}
                          className="w-7 h-7 rounded-md flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all"
                        >
                          <Plus size={12} />
                        </button>
                      </div>
                      ...
                      <div className="text-right">
                        <p className="font-bold text-white">{formatPrice(item.priceSnapshot * item.qty)}</p>
              ...
              {couponApplied ? (
                <div className="flex items-center justify-between p-3 bg-success/10 border border-success/30 rounded-xl">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Saving {formatPrice(couponDisc)}</p>
                  </div>
